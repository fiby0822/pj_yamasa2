...
class StoreFeatureGenerator:
    ...
    def generate_lag_features(self, df: pd.DataFrame, lags: List[int] = None) -> pd.DataFrame:
        ...
        # material_key（store_code）でグループ化
        for lag in lags:
    ...
    def generate_store_profile_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        店舗プロファイル特徴量を生成
        （store_code × 曜日などの特徴量は不要なので、店舗全体の統計量のみ）
        ...
        Returns:
            店舗プロファイル特徴量を追加したデータ
        """
        print("\nGenerating store profile features...")

        # 各店舗の全期間統計（学習データのみから計算すべき）
        store_stats = df.groupby('material_key')['actual_value'].agg([
            'mean', 'std', 'median', 'min', 'max'
        ]).reset_index()

        store_stats.columns = ['material_key'] + [f'store_{col}_f' for col in store_stats.columns[1:]]

        # マージ
        df = pd.merge(df, store_stats, on='material_key', how='left')

        print(f"Added store profile features")
    ...
    def save_features(self, df: pd.DataFrame):
        ...
        # タイムスタンプ付きファイル
        key_with_timestamp = f"output/features/store_level_features_{timestamp}.parquet"
        # 最新版
        key_latest = f"output/features/store_level_features_latest.parquet"

        for key in [key_with_timestamp, key_latest]:
        ...
        print(f"Saved successfully: {len(df)} rows, {df.shape[1]} columns")
    ...
    def run(self, train_end_date: str = "2024-12-31"):
        ...
        """
        print("="*60)
        print("Store-level Feature Generation")
        ...
        # 5. 店舗プロファイル特徴量
        df = self.generate_store_profile_features(df)
...
def main():
    """メイン処理"""
    generator = StoreFeatureGenerator(s3_bucket="fiby-yamasa-prediction-2")
