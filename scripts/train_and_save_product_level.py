...
class ProductLevelTrainer:
    ...
    def train_and_predict(self, df, train_end_date="2024-12-31", step_count=1):
        ...
        # TimeSeriesPredictor (S3保存無効化)
        predictor = TimeSeriesPredictor(
            bucket_name=None,  # S3保存を無効化
    ...
    def save_results(self, df_predictions, df_summary, feature_importance):
        ...
        pred_path = '/home/ubuntu/yamasa2/output/confirmed_order_demand_yamasa_predictions_latest_product_key.csv'
        df_predictions.to_csv(pred_path, index=False)
        print(f"Saved predictions: {pred_path}")

        # 2. material_summary_latest_product_key.csv
        summary_path = '/home/ubuntu/yamasa2/output/confirmed_order_demand_yamasa_material_summary_latest_product_key.csv'
        df_summary.to_csv(summary_path, index=False)
        print(f"Saved summary: {summary_path}")

        # 3. feature_importance_latest_product_key.csv
        fi_path = '/home/ubuntu/yamasa2/output/confirmed_order_demand_yamasa_feature_importance_latest_product_key.csv'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='商品レベルモデル学習・予測')
    parser.add_argument(
        '--train_end_date',
        type=str,
        default='2024-12-31',
    ...
    print(f"Step count: {args.step_count} month(s)")
    print("="*60)
    ...
    results = trainer.train_and_predict(df, train_end_date=args.train_end_date, step_count=args.step_count)
