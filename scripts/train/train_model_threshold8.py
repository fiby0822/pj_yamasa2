...
"""
モデル学習実行スクリプト（閾値8版）
テスト期間での実績発生数が8件以上のmaterial_keyを対象とする
"""
import sys
import argparse
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description='ヤマサ確定注文需要予測モデルの学習（閾値8版）')
    ...
    # train_test_predict_time_split 関数の呼び出し（閾値8を指定）
    df_pred_all, bykey_df, imp_last, best_params, model_last, metrics = predictor.train_test_predict_time_split(
        ...
        save_dir=args.save_dir if args.save_dir else "output/models/threshold8",
        verbose=True,
        min_test_active_records=8  # 閾値を8に設定
    )
    ...
    if len(df_pred_all) > 0:
        ...
        # 評価結果をファイルに保存
        import json
        evaluation_path = "output/evaluation/threshold8_metrics.json"
