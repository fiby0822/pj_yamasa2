...
"""
モデル学習実行スクリプト（閾値48版）
テスト期間での実績発生数が48件以上のmaterial_keyを対象とする
"""
import sys
import argparse
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description='ヤマサ確定注文需要予測モデルの学習（閾値48版）')
    ...
    df_pred_all, bykey_df, imp_last, best_params, model_last, metrics = predictor.train_test_predict_time_split(
        ...
        save_dir=args.save_dir if args.save_dir else "output/models/threshold48",
        verbose=True,
        min_test_active_records=48  # 閾値を48に設定
    )
    ...
    if len(df_pred_all) > 0:
        ...
        evaluation = evaluator.evaluate_predictions(
            ...
            output_prefix="threshold48"  # 出力ファイルのプレフィックス
        )
        ...
        # 評価結果をファイルに保存
        import json
        evaluation_path = "output/evaluation/threshold48_metrics.json"
