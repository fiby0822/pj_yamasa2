#!/usr/bin/env python3
"""
モデル学習実行スクリプト（閾値4版 = デフォルト）
テスト期間での実績発生数が4件以上のmaterial_keyを対象とする
"""
import sys
import argparse
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description='ヤマサ確定注文需要予測モデルの学習（閾値4版）')
    ...
    print("="*60)
    print(" ヤマサ確定注文需要予測 - モデル学習（閾値4版=デフォルト）")
    print("="*60)
    print()
    ...
    df_pred_all, bykey_df, imp_last, best_params, model_last, metrics = predictor.train_test_predict_time_split(
        ...
        save_dir=args.save_dir if args.save_dir else "output/models/threshold4",
        verbose=True
        # min_test_active_records は指定しない（デフォルトの step_count * 4 = 4 を使用）
    )
    ...
    if len(df_pred_all) > 0:
        ...
        # 評価結果をファイルに保存
        import json
        evaluation_path = "output/evaluation/threshold4_metrics.json"
